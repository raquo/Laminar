...
class SyntaxSpec extends UnitSpec {
  ...
  it("Sinks and Sources and assorted type inference") {
    ...
    div(
      ...
      text <-- boolBus,
      text <-- doubleBus.events,
      child.text <-- periodicInt, // legacy syntax – keep it alive for now, because it's in the video.
      text.maybe <-- textStream.map(Option(_)),
      text.maybe <-- periodicInt.map(Some(_)),
      text.maybe <-- boolBus.events.toWeakSignal,
