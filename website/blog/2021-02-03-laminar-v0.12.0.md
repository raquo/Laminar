...
This release is packed with new features and ergonomics improvements. See "migration" comments in this post for directions on breaking changes. Also, make sure to read the new / updated parts of the docs as linked below.
...
Thanks to `mapToValue` / `mapToChecked` and `composeEvents`, you will need `inContext` very rarely now.
...
* Laminar doesn't try to save you from yourself when you use `onClick.preventDefault` on checkboxes anymore. Previously if you registered a click listener on a checkbox and used the EventPropTransformation preventDefault operator on it, Laminar would call your observer **asynchronously** to make sure that the browser resets the checkbox state before your observer is run.
  ...
  * **Migration:** find usages like this in your code: `input(typ := "checkbox", onClick.preventDefault --> observer)` and use the new APIs instead of `preventDefault`.

* `events` method only accepts a single param now, an EventProcessor.
  * **Migration:** use EventProcessor operators to achieve what the optional params would, e.g. `events(onClick, useCapture = true)` -> `events(onClick.useCapture)`.

* Removed `ReactiveElement.addEventListener` / `removeEventListener` / `indexOfEventListener` methods.
  **Migration:** If you want the plain JS versions, use `DomApi`. Otherwise, use `bind` on `EventListener` to add and `kill` on the resulting `DynamicSubscription` to remove the listener.
...
* API: Reduce the number of `<--` and `-->` methods using new [Source & Sink](https://github.com/raquo/Airstream/#sources--sinks) types.
