...
div(
  ...
  div("stream1: ", text <-- stream1),
  div("stream2: ", text <-- stream2),
  div("stream3: ", text <-- stream3)
)
```
...
val zeroBasedIndex = oneBasedIndex.zoom(_ - 1)(_ + 1)(using owner)
...
* `StrictSignal`'s kill method was renamed to `killOriginalSubscription` and moved to the new subclass of `StrictSignal` called `OwnedSignal`. And so, `signal.observe(using owner)` now returns an OwnedSignal. Unless you actually used the kill method, you shouldn't need to change anything.
