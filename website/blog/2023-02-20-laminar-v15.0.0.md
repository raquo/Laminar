...
If you use this new `flatMap` method in IntelliJ IDEA, you'll be annoyed to find that it causes the IDE to incorrectly report a fake type error. Not holding my breath for this being fixed any time soon, so I added more specialized `flatMapStream` and `flatMapSignal` methods which don't trigger the fake error in the IDE.
...
Laminar has many convenience helpers to reduce the boilerplate around the `-->` methods. You should probably read all the Laminar docs on event handling, because I often see people not knowing the shortcuts.

Currently you can put any `A => ()` or `Sink[A]` on the right hand side of `-->` methods, including `Observer[A]`, `EventBus[A]`, and `Var[A]` (pro tip – you don't even need `.writer`, that's oldskool).
...
Unfortunately some of these helpers suffer from being unintuitive, and require good understanding of Laminar types to use effectively. For example, `myVar.updater` is basically a variation of its `update` method that returns an Observer. `onClick --> { _ => println("hello") }` is annoyingly verbose, and `onClick.mapTo("hello") --> println` splits the responsibility in a rather weird way unless you're used to a certain style of programming.
...
This kind of API is sometimes frowned upon because users can accidentally pass a non-Unit value that does not perform the desired side effect on its own (such as IO), or a value that is "surprisingly" typed as Unit (such as `if (foo) nonUnit` in Scala 3). For this particular API, this is actually not possible in Scala 2, the only problem is that else-less `if`-s do not behave like proper expressions anymore, not returning their content. There is also no way to get a compiler warning about an unused value inside the `if` branch, unless it's a trivial pure value like `5`.

The risk of this new Laminar API is that users might try to make something like this:

```scala
div(onClick.mapTo(5) --> myIntVar)
```

Conditional on `foo` like this:

```scala
div(onClick.mapTo(5) --> (if (foo) myIntVar.writer))
```

Which would compile, but in Scala 3 it would not work, because in Scala 3 the `if` expression returns `()` instead of `myIntVar.writer`.

Meanwhile, any of the following would be the correct way to add conditional execution:

```scala
div(onClick --> (if (foo) myIntVar.set(5)))
div(onClick.filter(_ => foo) --> myIntVar.set(5))
div(onClick.mapTo(5) --> (v => if (foo) myIntVar.set(v)))
div(onClick.mapTo(5).filter(_ => foo) --> myIntVar.writer)
div(onClick.mapTo(5) --> myIntVar.writer.filter(_ => foo))
```

For now, I think that the risk of such misuse is pretty low, and the benefit of reducing boilerplate is significant enough to justify this feature. We'll see how it works in practice, and re-evaluate. We could potentially hide it behind a special implicit import, but I am very reluctant to create several variants of Laminar's syntax.
...
## Changes Only Relevant To Airstream Extensions
...
* Laminar `Key` types like `ReactiveProp`, `ReactiveHtmlAttr`, etc. don’t extend the corresponding `com.raquo.domtypes` types anymore (those types were removed from _Scala DOM Types_), and were also renamed to remove the “Reactive” prefix.
