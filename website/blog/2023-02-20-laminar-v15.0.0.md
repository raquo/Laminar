...
This kind of API is sometimes frowned upon because users can accidentally pass a non-Unit value that does not perform the desired side effect on its own (such as IO), or a value that is "surprisingly" typed as Unit (such as `if (foo) nonUnit` in Scala 3). For this particular API, this is actually not possible in Scala 2, the only problem is that in Scala 3 `else`-less `if`-s do not behave like proper expressions anymore – they no longer return their content. There is also no way to get a compiler warning about an unused non-Unit value inside the `if` branch, unless it's a trivial pure value like `5`.

So, in Scala 3, with the new Laminar API, we risk that users might start with something like this:

```scala
div(onClick.mapTo(5) --> myIntVar)
```

And then try to make it conditional on `foo` like this:

```scala
div(onClick.mapTo(5) --> (if (foo) myIntVar.writer))
```

Which does not make sense if you know Laminar, but it would compile, yet in Scala 3 it would not work, because in Scala 3 the `if` expression returns `()` instead of `myIntVar.writer`.
