...
This kind of API is sometimes frowned upon because users can accidentally pass a non-Unit value that does not perform the desired side effect on its own (such as IO), or a value that is "surprisingly" typed as Unit (such as `if (foo) nonUnit` in Scala 3). However, **Laminar's API does not seem to suffer from this shortcoming** – you can look at the `"unit-based binding syntax"` test in Laminar's `SyntaxSpec` test suite to see everything that I tried to throw at it, and the bad code just doesn't compile, without the need for special compiler options. If you can make this API produce unexpected results somehow, please let me know, I would like to know the trick, and I am open to reviewing the API.
